...
const ContactPage = () => {
  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      // Find exact matches case-insensitively or matching exactly
  ...
  const handleSubmit = (e) => {
    ...
    console.log('EmailJS Config Check (ContactPage):', {
    ...
    emailjs.send(serviceID, templateID, templateParams, {
      ...
      .then((response) => {
        console.log('EmailJS Response (ContactPage):', response);
        if (response.status !== 200) {
          throw new Error(`EmailJS responded with non-success status: ${response.status} (${response.text})`);
        }

        console.log('EmailJS Send Success (ContactPage):', {
      ...
      .catch((err) => {
        console.error('EmailJS Send Failure Details (ContactPage):', err);
  ...
  const contactInfos = [
    ...
    { icon: MapPin, label: 'Our Location', value: 'Madurai, Tamil Nadu, India' },
  ];

  return (
    <PageTransition>
      <div className="relative min-h-screen w-full flex flex-col overflow-hidden bg-[#F8FAFC] text-text-secondary">
                    ...
                    {status === 'success' ? (
                      ...
                      >
                        ...
                        <div className="relative text-left">
                          ...
                          <label
                            htmlFor="name"
                            className={`absolute left-4 transition-all duration-300 pointer-events-none font-inter font-semibold ${formData.name
                        ...
                        <div className="relative text-left">
                          ...
                          <label
                            htmlFor="email"
                            className={`absolute left-4 transition-all duration-300 pointer-events-none font-inter font-semibold ${formData.email
                                ? 'top-0 text-xs text-text-tertiary bg-white px-1.5 -translate-y-1/2 peer-focus:text-brand-primary'
                                : 'top-1/2 -translate-y-1/2 text-sm text-text-tertiary peer-focus:top-0 peer-focus:text-xs peer-focus:text-brand-primary peer-focus:bg-white peer-focus:px-1.5 peer-focus:-translate-y-1/2'
                              }`}
                          >
                        ...
                        <div className="relative text-left">
                          <input
                            ...
                            disabled={status === 'sending'}
                            className={`peer w-full px-4 py-3.5 rounded-xl border ${phoneError && phoneTouched ? 'border-red-500 focus:border-red-500' : 'border-slate-200 focus:border-brand-primary/40'
                              } bg-white text-slate-900 font-inter text-sm outline-none transition-all duration-300 shadow-sm`}
                          />
                          <label
                            htmlFor="phone"
                            className={`absolute left-4 transition-all duration-300 pointer-events-none font-inter font-semibold ${formData.phone
                                ? 'top-0 text-xs text-text-tertiary bg-white px-1.5 -translate-y-1/2 peer-focus:text-brand-primary'
                                : 'top-1/2 -translate-y-1/2 text-sm text-text-tertiary peer-focus:top-0 peer-focus:text-xs peer-focus:text-brand-primary peer-focus:bg-white peer-focus:px-1.5 peer-focus:-translate-y-1/2'
                              }`}
                          >
                        ...
                        <div className="relative text-left">
                          ...
                          <label
                            htmlFor="service"
                            className={`absolute left-4 transition-all duration-300 pointer-events-none font-inter font-semibold ${formData.service
                                ? 'top-0 text-xs text-text-tertiary bg-white px-1.5 -translate-y-1/2 peer-focus:text-brand-primary'
                                : 'top-1/2 -translate-y-1/2 text-sm text-text-tertiary peer-focus:top-0 peer-focus:text-xs peer-focus:text-brand-primary peer-focus:bg-white peer-focus:px-1.5 peer-focus:-translate-y-1/2'
                              }`}
                          >
                        ...
                        <div className="relative text-left">
                          ...
                          <label
                            htmlFor="message"
                            className={`absolute left-4 transition-all duration-300 pointer-events-none font-inter font-semibold ${formData.message
                                ? 'top-0 text-xs text-text-tertiary bg-white px-1.5 -translate-y-1/2 peer-focus:text-brand-primary'
